...
\newcommand\watermath  {H^{2}O}
\newcommand\ozone    {O$^3$}
\newcommand\ozonemath {O^{3}}
\newcommand\oxy     {O$^2$}
\newcommand\oxymath  {O^{2}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
(the $A_{rayleigh/\oxymath/\ozonemath/\watermath}$ and $A_{aerosol} = 1-e^{\tau_{aerosol}}$ from
...
S^{fit}(alt,az,t, \lambda) & = & \,e^{-\tau_{aerosol}(alt,az,t,\lambda)\,X} \nonumber \\
 ...
 & & \times \, (1 - \sqrt{ C_{mol}\, (BP(t) / BP_o)} \, A_{\oxymath}(X, \lambda)) \nonumber \\
 & & \times \, (1 - C_{\ozonemath}(t) \, A_{\ozonemath}(X, \lambda)) \nonumber \\
 & & \times \, (1 - C_{\watermath}(alt,az,t)\,A_{\watermath}(X, \lambda)).
\label{eqn:atmo_fit}
\end{eqnarray}
The $A_{Rayleigh/\oxymath/\ozonemath/\watermath}$ functions are
absorption templates (i.e. 1 minus the transmission profiles from the
MODTRAN models) and the $C_{mol,\ozonemath,\watermath}$ are
...
$C_{mol,\ozonemath,\watermath}$, increasing our SNR for these
...
\ozone absorption is fit with a single $C_{\ozonemath}$ value for each
...
\begin{equation}
\label{eqn:coeff_h2o}
C_{\watermath}(alt,az,t) = C_{\watermath}(t) + {dC_{\watermath} \over d{\rm EW}}\,{\rm EW} +
{dC_{\watermath} \over d{\rm NS}}\,{\rm NS}
\end{equation}
...
The coefficients $C_{mol/\ozonemath/\watermath}$ and $\tau^{aerosol}$ will be
...
$C_{mol/\ozonemath/\watermath}$ and $\tau^{aerosol}$ are better
...
$C_{\ozonemath}$, $C_{\watermath}$, $\tau_i$ and $\alpha$ parameters from these runs,
...
$C_{\watermath}$ only affects the $z$ and $y$ bands, while changing $C_{\ozonemath}$,
$\tau_0$, and $\alpha$ affect the $u$ and $g$ and, to a lesser extent, $r$
bands. Using these `worst-case' parameters, at $X=1.2$ we find
differences of $\Delta u$=14~mmag, $\Delta g$=7~mmag, $\Delta r$=4.5~mmag,
...
achieving 10\% accuracy on $\tau^{aerosol}$, $C_{\ozonemath}$, and
$C_{mol}$, and 30\% accuracy for $C_\watermath$, we can correct for
...
Changes in airmass have a larger effect on observed magnitudes
