...
\begin{figure}[htbp]
\includegraphics[width=6in]{ghost_images}
\caption{ {\small
...
ZEMAX model would suffice to create an illumination corrected
synthetic flat.  We can also create a preliminary model of the stray
light scattered into the LSST etendue using work similar to that of
Photon Engineering, Inc., which can be improved by taking measurements
of the focal plane response while blacking out M2 (to image only the
stray light), improving the estimate of the illumination
correction. With the dome projectors providing illumination uniform to
within 1\% across the field of view, the resulting preliminary
illumination correction estimate could be accurate to within a few
percent, however unexpected light scattering outside the camera
body(such as from an unexpected surface like the edge of a filter, the
filter holder or other potentially bright surface) would not yet be
accounted for.

To determine the final illumination correction, a dense network of
stars of a variety of spectral types must be rastored across the focal plane
on a photometric night. The images will be divided by the synthetic flat field
corrected by the preliminary illumination corrected, and then the counts
for each star corrected for varying color terms in the atmosphere and 
hardware response, as further described in subsection~\ref{sec:phi_correction}. 
The final update to the illumination correction can then be determined by
...
Similar rastor scans have been successfully used in previous surveys,
such as \citet{Regnault2009, Magnier2004, Manfroid1996} at the few
percent level.  With the addition of the CCOB and careful design of
the dome screen projectors to the generation of the illumation correction, 
the final synthetic flat field (with illumination correction) is expected to
carry an error of less than 0.25\% , or 2.5 mmag, on scales less than
approximately the size of a CCD and less than 0.4\%, or 4 mmag, across
