...
Section~\ref{sec:uncertainty_budget} presents the uncertainty budget for each step of the 
...
and below are discussed further in Section \ref{sec:uncertainty_budget} on the 
...
\ref{sec:uncertainty_budget} and \ref{sec:verification}
...
transmisions, and detector sensitivities. The term
...
radius in the focal plane and star color, for a set of Kurucz main sequence stars.   The effects are clearly too large to ignore, or they would overwhelm the uncertainty budget.   This is discussed further in Section \ref{sec:uncertainty_budget}.  It seems likely that the quality 
...
we must use the observations of calibration stars in the images themselves, as
 observations (\citep{Burke2013}, \citep{Ivezic2007}) suggest that cloud extinction can vary by 0.01 magnitudes on the scale of a CCD
...
\item{Acquire a broadband flat in each filter at the start of
...
(\citep{Blake2011}, \citep{Radomski2010}, \citep{Gaffard2003}).  Their data will be logged through the Engineering and Facility Data Base. 
...
systematic limits of the photometry pipeline as well as constrian the
...
accross the sky.  \citet{Ivezic04} use a principal color analysis,
...
two separate productions, described briefly here, and in much more detail in DM publications {\bf NEED REFS for DM}.
...
\caption{{\small
The flat field illumination system (PLACEHOLDER)
}}
