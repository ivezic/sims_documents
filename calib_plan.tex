...
positioning to remain less than a 0.5~mmag source of error. 
%Given the
...
response and thus contributing directly to the final error in the calibration
of the observed magnitudes. Note that the y scale here is 1/10th 
...
will be interpolated from atmospheric extinction coefficients fit to a
series of spectroscopic measurements of atmospheric absorption
gathered by the auxiliary telescope.  It is worth noting that the
auxiliary telescope measurements will not correct for cloud extinction
or any other normalization variation due to the atmosphere; the goal
for the auxiliary telescope to compensate for changes in the shape of
the atmospheric transmission curve only.
...
transmission curves that scale almost linearly with airmass,
$S(\lambda, X) \propto e^{-\tau X}$.  Molecular absorption produces a
more complex set of absorption bands and features, originating from
line absorption due to ozone (O3), oxygen (O2), water (H2O) and other
trace elements (OH, N2O, etc.).  The molecular absorption is largely
due to narrow saturated Lorentzian-shaped lines spaced closely in
wavelength and thus scales closer to the square root of the airmass,
$S(\lambda, X) \propto e^{-\tau\,\sqrt(X)}$
\citep{Burke2010b}. Figure~\ref{fig:absorption_comps} demonstrates the
...
will depend on airmass, and the aerosol scattering profile will vary
slightly with time.  }}
\label{fig:absorption_comps}
\end{figure}

The contribution of each component toward the final total atmospheric
absorption 
We expect the Rayleigh scattering to be axisymmetric about the zenith,
with a time dependence driven only by pressure variations. The O2
...
for various locations in the focal plane in each exposure. 
