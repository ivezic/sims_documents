...
def make_pixgrid(sensitivity_var=0.002):
...
def add_star(star, rawdetector, pixscale_change=-0.005):
...
def measure_star(pixgrid, star, pixscale_change=-0.005):
    starx = star[0]
    stary = star[1]
    starflux = star[2]
...
def add_domewvariance(pixgrid, domeval, domegradient=0.01):
...
def add_pixscale(pixgrid, pixscale_change=-0.005):
...
def add_ghost(pixgrid, rawdetector, ghostscale=1e-3, ghostring_inner=skysize/2.7, ghostring_outer=skysize/1.2):
    ...
    ghostarea = (ghostring_outer**2 - ghostring_inner**2)*numpy.pi
    ...
    for i in range(0, len(pixgrid[0])):
        for j in range(0, len(pixgrid[1])):
            distance = numpy.sqrt((i-pixcenX)**2 + (j-pixcenY)**2)
            if distance>ghostring_outer:
                ghostring[i][j] = 0
            elif distance<ghostring_inner:
                ghostring[i][j] = 0
            else:
...
def make_narrowbandflat():
    # make the blue flat field
    domeval = 10000
    detector = make_pixgrid()
    flat = {}
    ...
    fig.subplots_adjust(wspace=0.25, hspace=0.01, left=0.08, right=0.94)
    ax = pylab.subplot(231)
    pylab.title("'Blue' narrow band flat", fontsize='medium')
    im = ax.imshow(flat['blue'], origin='lower')
    pylab.colorbar(im, shrink=0.65, format="%.3f")
    pylab.grid(which='major')
    pylab.xlim(0,skysize-1)
    pylab.ylim(0,skysize-1)
    ax.tick_params(axis='x', labelbottom='off')
    ax = pylab.subplot(232)
    pylab.title("'Green' narrow band flat", fontsize='medium')
    im = ax.imshow(flat['green'], origin='lower')
    pylab.colorbar(im, shrink=0.65, format='%.3f')
    pylab.grid(which='major')
    pylab.xlim(0,skysize-1)
    pylab.ylim(0,skysize-1)
    ...
    ax.tick_params(axis='y', labelleft='off')
    ax = pylab.subplot(233)
    pylab.title("'Red' narrow band flat", fontsize='medium')
    im = ax.imshow(flat['red'], origin='lower')
    pylab.colorbar(im, shrink=0.65, format='%.3f')
    ax.tick_params(axis='x', labelbottom='off')
    ax.tick_params(axis='y', labelleft='off')
    pylab.grid(which='major')
    ...
    pylab.ylim(0,skysize-1)
    ax = pylab.subplot(234)
    pylab.title("Synthetic Flat - blue", fontsize='medium')
    comboflat = 1.5*flat['blue'] + flat['green'] + 0.5*flat['red']
    comboflat = comboflat / comboflat.mean()
    im = ax.imshow(comboflat, origin='lower')
    pylab.colorbar(im, shrink=0.65, format='%.3f')
    pylab.grid(which='major')
    pylab.xlim(0,skysize-1)
    pylab.ylim(0,skysize-1)    
    pylab.xticks(rotation=-45)
    ax = pylab.subplot(235)
    pylab.title("Synthetic Flat - flat", fontsize='medium')
    comboflat = flat['blue'] + flat['green'] + flat['red']
    comboflat = comboflat / comboflat.mean()
    im = ax.imshow(comboflat, origin='lower')
    pylab.colorbar(im, shrink=0.65, format='%.3f')
    pylab.grid(which='major')
    ...
    pylab.xticks(rotation=-45)
    ax = pylab.subplot(236)
    pylab.title("Synthetic Flat - red", fontsize='medium')
    comboflat = 0.5*flat['blue'] + flat['green'] + 1.5*flat['red']
    comboflat = comboflat / comboflat.mean()    
    im = ax.imshow(comboflat, origin='lower')
    pylab.colorbar(im, shrink=0.65, format='%.3f')
    pylab.grid(which='major')
