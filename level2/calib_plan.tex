...
\subsection{Effects of PSF Width}
...
focal plane.  We must now account for the fact that the system has a PSF with finite width, so that the flux from an object is spread
...
\end{equation}
Further, define the SED of the object, $f(\lambda)$ by

\begin{equation}
F_\nu(\lambda) = F_b f_\nu(\lambda)
\end{equation}

where $\lambda$ must be within band b, and
...
\begin{equation}
Q_{b,i} = \int {q_{b,i}(\lambda) f(\lambda) d\lambda}
\end{equation}
...
At this point, we notice that $Q_{b,i}$ is what we mean by a 'flat', but there is a problem:  At the time we process the pixels
...
of $f(\lambda)$.  Even if we knew it, this prescription would be unworkable because the flat would need to be constructed with an SED 
...
content ourselves with the broadband flat

\begin{equation}
Q_{b,i}^{BB} = \int {q_{b,i}(\lambda) f^{BB}(\lambda) d\lambda}
\end{equation}

where $f^{BB}(\lambda)$ is an SED we are free to choose in constructing the broadband flat. We can now write

\begin{equation}
F_b = \left[ {C_b^{inst} \over {\sum_i{w_i Q_{b,i}^{BB}}}}\right]  \left[ {\sum_i {w_i Q_{b,i}^{BB} } \over {\sum_i {w_i Q_{b,i} }\right]
\end{equation}
...
of the pixel within the PSF footprint.

We have a final problem:  $Q_{b,i}^{BB}$ depends on $S_b^{atm}$.

We now turn to the actual measurement of $q_{b,i}$, and the construction of $Q_{b,i}^{BB}$
...
%In the first stage of the calibration process, that which
%processes the actual pixels, we assume that sources have flat SEDs within each filter band, so we construct our synthetic flat
%with a flat SED.  We call this flat the 'flat spectrum flat' (FSF).
...
which must later be corrected for (in the SED Correction stage, see Figure \ref{fig:overview_flowchart1}) 
when both the true $\phi_b^{obs}(\lambda)$ and the actual object SED, $f_\nu(\lambda)$ are known.  The correction takes the form

\begin{equation}
m_b^{nat*} = m_b^{nat} -2.5 \, log_{10} \,  \left( { \int_0^\infty {f_\nu^{bbf}(\lambda,t) \,
    \phi_b^{obs}(\lambda,t) \, d\lambda} \over \int_0^\infty {f_\nu(\lambda,t) \,
    \phi_b^{obs}(\lambda,t) \, d\lambda}} \right)
\label{eqn:bbf_corr}
\end{equation}

Note the similarity to equation \ref{eq:xx} for $\Delta m_b^{obs}$, but also note that here we are correcting for different SEDs 
through the same $\phi$ versus correcting for different $\phi$s at a fixed SED.   Given the arbitrariness of $f_\nu^{bbf}(\lambda)$,
we must choose on grounds of convenience.  At least two choices suggest themselves:

\begin{itemize}
\item{Choose $f_\nu^{bbf}(\lambda) = const$.  This makes the integral in the numerator of equation \ref{eqn:bbf_corr} unity.}
\item{Choose $f_\nu^{bbf}(\lambda)$ to be in some sense an average SED for all objects in the survey, such that the average value
of the BBF correction is minimized.}
\end{itemize}

We choose the first of these, and subsequently refer to the BBF as the FSF (flat spectrum flat).
...
oceanographic and meteorological communities, and based on the Gauss-Markov theorem (\citep{Bretherton1976}, \citep{McIntosh1990}).
