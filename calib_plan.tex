...
\subsubsection{Normalized bandpass response}
\label{sec:phi}
...
\end{equation}. 
Using $\phi_b^{obs}(\lambda, t)$ we can represent the (true)
...
vary with changes in $\phi_b^{obs}(\lambda,t)$ due only to changes in the
bandpass shape with position in the focal plane or differing
atmospheric absorption characteristics, {\it even if the source is
  non-variable}. 

For this reason, we also introduce a standardized bandpass response
...
Okay so here - write how F_b^{obs} varies observation to observation
and introduce standard bandpass. 
Then introduce magnitude (standard) and natural. Then add Zeropoint
and tie back to observed counts! Tada. Drop next few scary equations. 

XXXX


Combining Equations~\ref{eqn:Fpupil} and
~\ref{eqn:Fpupil2counts}, 
\begin{equation}
...
\begin{center}
\begin{table}[htb]
\caption{{\bf Changes in observed magnitudes due to variations in system and atmospheric
...
Standard atm, +1\% sys shift & red  & -31 & -22 & -8 & -2 & 1 & 1 \\
Standard atm, +1\% sys shift & blue  & 9 & 17 & 20 & 20 & 16 & 16 \\ \hline
