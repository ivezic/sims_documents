...
\item Laboratory measurements of the as-built camera with the CCOB (see Appendix X)
\item {Raster scans of star fields in photometric conditions.  The stars will be chosen
to be dense, without compromising photometry, and to span a wide range of colors.}
...
\fbox{
\begin{minipage}{6.5in}
\includegraphics[width=6in]{AtmoFigs/BurkeObsPattern.png}
\caption{ {Pattern of observing atmospheric probe stars from \citep{Burke2010}.  The Solid lines trace
...
As sketched in Figure \ref{fig:overview_flowchart1}, $m_b^{inst}$ is produced by the LSST Data Management System (DM), through processing raw science exposures. Leading error contributions come from the flat spectrum flat (modulated by position
...
\item{Errors in the atmospheric bandpass, $S^{atm}$
	...
	{\bf MISSING section on aux tel errors into atm model errors}
