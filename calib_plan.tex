...
for the bulk of the remainder of the calibration procedures.  The
quasar locus will also provide an additional check on systematic
effects, particularly useful due to the fact that quasars will exhibit 
different levels of dust extinction through the sky than either the
main sequence stellar locus or the white dwarfs.
...
these targets has not yet been completed. 
...
$r=16$ to $r=20$.  After using synthetic photometry to transform these
stars from Gaia bandpasses to LSST $r$ band, the magnitude measurements of stars
calibrated with the LSST calibration procedure described in this
document can be compared with the measurements from Gaia (and
translated to the LSST bandpass). The residuals between the LSST
measurements and the Gaia measurements can be fit using a variety of
methods and then evaluated for calibration to an external system, 
as well as providing a check on the overall uniformity of the LSST
...
\item{Hardware response curve - $\phi_b^{sys}(\lambda,t)$. There is a
    narrow band dome screen projector system installed at PanStarrs
