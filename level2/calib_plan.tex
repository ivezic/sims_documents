...
fields'.  A similar approach has already been employed at PS-1 (\citep{StubbsTonry2012})
...
\begin{minipage}{6.5in}
\begin{tabular}{ccc}
\subfloat[]{\includegraphics[width=2in]{IllumCorrFigs/camera_ghosting_ff_calibbias_jdsu_lsstcone_121217_u_ghosting.png}}
	& \subfloat[]{\includegraphics[width=2in]{IllumCorrFigs/camera_ghosting_ff_calibbias_jdsu_lsstcone_121217_g_ghosting.png}} 
	& \subfloat[]{\includegraphics[width=2in]{IllumCorrFigs/camera_ghosting_ff_calibbias_jdsu_lsstcone_121217_r_ghosting.png}} \\
\subfloat[]{\includegraphics[width=2in]{IllumCorrFigs/camera_ghosting_ff_calibbias_jdsu_lsstcone_121217_i_ghosting.png}}
	& \subfloat[]{\includegraphics[width=2in]{IllumCorrFigs/camera_ghosting_ff_calibbias_jdsu_lsstcone_121217_z_ghosting.png}}
	& \subfloat[]{\includegraphics[width=2in]{IllumCorrFigs/camera_ghosting_ff_calibbias_jdsu_lsstcone_121217_y4_ghosting.png}} \\
\end{tabular}	
...
Because the illumination correction is wavelength dependent (highly so near the edges of the filter bandpasses), its effect on photometry is dependent on the SED of the object in question.  Figure \ref{fig:FRED_Kurucz} shows this effect as a function of both 
...
Many sources of errors feed into the calibration process, and it is a challenging task to identify and quantify all of them.  We first
...
%Have the NIST calibration data}
\item{Errors in determining the SEDs of calibration stars from their multicolor photometry.
%Need intrinsic width of color-color relationship.  Then, need to translate that width into a change in Delta}
\end{itemize}

\subsubsection{Errors in $Z_b^{obs}$}
Every exposure has a spatial zeropoint model determined by Self Calibration as the solution to a large linear least squares problem, as discussed in Section \ref{sec:counts2photons}.  The errors in the zeropoints are linearly related to the zeropoints themselves (is this really true?), and we roughly model this relationship as an attenuation factor.  The attenuation factor is dependent on the spatial scale of the perturbations...  We have determined the attenuation factor from simulations of the Self Calibration process (Section \ref{sec:verification}).   Additionally, Gauss-Markov analysis (show results?)
...
Affected term & Effect &  $u$  & $griz$ & $y$ \\ \hline
{\bf $m_b^{obs}$} & {\bf Total} & 3.0 & 3.0 & 3.0 \\ \hline
{\bf $\Delta m_b^{obs}$} & & & & \\
& Spectrum errors & & & \\
& Undetected atmospheric variability & & & \\
& Monochromatic illumination correction errors & 3.0 & 0.6 & 0.6\\
...
& Errors in standards & & & \\
& {\bf Total} & & & \\ \hline
{\bf Total} & & 4.2 & 3.1 & 3.1 \\ \hline
{\bf Requirement} & & 7.5 & 5.0 & 7.5 \\ 
\end{tabular}
\label{tab:delta_mags}
\end{table}
\end{center}
