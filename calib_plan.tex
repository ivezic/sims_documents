...
\caption{The flat field obtained from the dome screen (whether in a
...
\caption{The two rows in this figure illustrate the necessity of the
  illumination correction. The top row shows the result of correcting
  a raw sky image with a dome flat without an illumination correction,
  while the bottom row demonstrates the result of correcting a raw sky
  image with a dome flat which has been multiplied by an illumination
  correction. 

The left top panel shows a flat field obtained from a dome screen,
creating with the same conditions as
in Figure~\ref{fig:flatfield}. The flat field has been left `as is'
and no illumination correction has been added. The central top panel
shows a raw `image' of the sky, generated by adding a background sky
value of 1500 counts per pixel (scaled by the pixel area, using the
0.5\% variation in pixel scale from the center to the edge of the
field of view) to two stars. The stars were generated by spreading
5000 counts over a circular aperture the size of the PSF at the
location of the star. A ghost image was created by spreading
$1e{-3}$ times the total light in the image over a ring, as in
Figure~\ref{fig:flatfield}. 
The right top panel shows what happens when this raw sky image is
processed by subtracting a model ghost image (appropriate for the sky
level) and then
divided by the dome flat, without any illumination correction. Note
that the variation from the dome flat imprints into the result, as
well as the ghost image from the dome flat, and
that the variation in the sky background due to variations in pixel
area have been removed by the similar pattern in the flat. 
