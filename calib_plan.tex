...
different filters together, enabling precision measurement of colors. 
\label{color_req}}
...
filter. Performance of the LSST system regarding requirement
...
(to test whether the observational cadence itself is suitable for
the self-calibration procedure described below), comparisons to
known standards, and evaluation of science outputs such as stellar
locus diagrams. These last two are also relevant to verifying the
final two requirements, \ref{color_req} and \ref{abs_req}. 
Requirements \ref{color_req} and \ref{abs_req} require
...
self-calibration procedure will primarily correct for variations in throughput
...
atmosphere. Unless $F_\nu(\lambda,t)$ is a flat SED, $F_b^{obs}$ will
vary with changes in $\phi_b^{obs}(\lambda,t)$ due simply to changes in the
bandpass shape, such as changes with position in the focal plane or differing
...
changes. The natural magnitude can be translated to a `standard
magnitude', $m_b^{std}$, as follows: 
\begin{eqnarray}
m_b^{nat} & = &-2.5\, log_{10} \, {F_b^{obs} \over F_{AB}}  \\
...
$\phi_b^{obs}(\lambda,t)$ in each observation.  For non-variable
sources, $m_b^{std}$ will be non-variable as it represents the
throughput in a standardized bandpass,
$\phi_b^{std}(\lambda)$.  
...
\item{{\bf Normalized system response, $\phi_b(\lambda)$.} The normalized
    system response describes the shape of the bandpass transmission
    curve, separating this from the normalization of the throughput
    curve which can be determined separately.  $phi_b(\lambda)$ is
    described by Equation~\ref{eqn:PhiDef}. }
