...
throughput curve, $S_b^{sys}(\lambda,x,y,t)$. We can measure the
rapidly time variable gray-scale relative normalization changes in the hardware
using standard white-light flat fields acquired at the start and end
of each night through each filter, in a manner similar to traditional
dome flats. This is discussed in detail in section~\ref{sec:hardware_norm}. 
To measure the wavelength-dependent shape of the hardware
response curve as a function of position in the focal plane, we will
use a dome-screen system that is capable of producing light at a range
of individual wavelengths - producing a data cube of `narrow band flat
...
the more slowly variable $\phi_b^{sys}(\lambda,t)$. This measurement is discussed
in detail in section~\ref{sec:hardware_phi}. These calibration
stages particularly important at small size scales, as no later
calibration stage can contribute to corrections at scales less than
the Point Spread Function (PSF) because these later stages depend on the integrated
...
illumination correction must correct the observed flat fields for the
effects of non-uniform illumination of the focal plane by the dome
screen, for the effect of pixel scale variations across the field of
view, for ghosting caused by internal reflections in the camera, and
for the presence of stray or scattered light arriving in the focal
plane on non-optical paths. See figures~\ref{fig:flatfield} and
\ref{fig:iceffect} for a visual example of the illumination correction
and its importance to image processing.  Particularly important for
the narrow band flat fields, the observed ghosting pattern in each flat field is 
wavelength dependent, so a separate correction for each
wavelength must be measured. The illumination correction
(including ghost corrections) will be generated by combining
measurements of bright, dense star fields rastered across the field of
view during specialized observing sequences and models of the
ghost patterns at various wavelengths constrained by measurements obtained
with the `camera calibration optical bench' (CCOB), with the
observed flat fields. The illumination correction is expected to be
stable with time and will be remeasured only when the optical path of
the telescope is altered. The generation of the illumination correction, 
including a discussion on ghosting, is discussed further in section~\ref{sec:ic}. 
...
section~\ref{sec:atmo_phi}.  In order to correct for the higher
...
correction) may be possible to determine on scales down to a few times
...
hardware transmission ($S_b^{sys}$) will be done using a white-light flat field 
generated using a broad band source illuminating the specialized dome screen projector
discussed further in section~\ref{sec:hardware_phi}. For the white-light
flat field generation, the relevant pieces of information are that 
the `white-light' source can be tuned to have a chosen spectral profile and that 
the dome screen is required to provide very uniform illumination to the LSST etendue. 
[blah, what the spectral profile might/should be - night sky seems good choice?-, 
how it will be chosen. also, what the uniformity specs are, and mention these specs 
also call for minimizing stray light.]

This is the first step in photometric calibration and is necessary to
correct for variations in the normalization of $S_b^{sys}$ that are
smaller than a few times the PSF (and in the current calibration
implementation, smaller than the scale of a CCD).
...
narrowband flat fields described in section~\ref{sec:hardware_norm}. At
