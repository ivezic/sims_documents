...
the atmospheric throughput. The telescope hardware gray-scale
zeropoint will be measured with a broadband flat field (detailed in
section~\ref{sec:hardware_norm}), while the hardware throughput
response curve shape will be measured with narrow-band flat fields
(detailed in section~\ref{sec:hardware_phi}). The atmospheric
gray-scale zeropoint will be calculated using the self-calibration
procedure (detailed in section~\ref{sec:atmo_norm}), while the
atmospheric response curve shape will be measured with the auxiliary
telescope (detailed in section~\ref{sec:atmo_phi}).  The separation of
the normalization and shape of the throughput curve, as well as the
separation of the telescope hardware from the atmospheric throughput,
allows the use of methods optimized to measure the effect of each
individual component of the system at the appropriate spatial and
temporal scales, allowing us to achieve photometric calibration at the
required levels.
...
bench' (CCOB) and/or with measurements where the dome screen was
illuminated in narrow-band mode at a single location only.  Further
corrections will be generated by the self-calibration stage (discussed
...
a) and at airmass=2.0 (panel b) is shown here.  The \water\,(blue) and
...
\begin{figure}
\centering
\subfloat[a]{\includegraphics[width=3.2in]{Stardensity}}
\subfloat[b]{\includegraphics[width=3.2in]{Sdmag}} \\
\subfloat[c]{\includegraphics[width=3.2in]{StatsTruemBest}}
\subfloat[d]{\includegraphics[width=3.2in]{StatsRMSStars}} 
\caption{{\small
...
images, including the non-photometric data. 
...
image at the South Galactic Pole, although in theory only one of
well-measured standard would be required. These WDs will be recognizable from
color-color plots (see \citet{Ivezic2007} Figure 24 for an example).
...
\item{{\bf Camera Calibration Optical Bench (CCOB)}. The CCOB provides a
...
the filters and the camera shutter. The CCOB provides test data about
the camera assembly for camera acceptance and to constrain the optical
ZEMAX model. More details about the requirements and
