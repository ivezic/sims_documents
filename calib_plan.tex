...
only requirement is then that the quartz lamps do not change their
spectral profile (which may change the ghosting pattern) or
illumination pattern by more than an amount which would cause a change
of more than 0.1\% in the illumination of the white light flat at any
scale (that is, the illumination of the dome screen projectors with
the quartz lamps could be non-uniform by a larger amount, but the
pattern must be stable over time to 0.1\%).


\subsection{Shape of the Hardware and Atmospheric Response, $\phi_b^{atm+sys}(\lambda)$}
\label{sec:phi_correction}

The compensation for changes in observed magnitudes caused by
variations in the wavelength dependence (shape) of the hardware
and atmospheric response curves, $\phi_b^{sys}(\lambda)$ and
...
atmospheric response curve from MODTRAN models based on measurements
from the auxiliary telescope.  While the measurements are independent,
the actual correction that must be applied depends on the combination
of atmospheric and hardware response curves as well as the SED of the
astronomical objects.  This correction is necessary for precision
photometry but as it requires knowledge of the object's SED, general
LSST reported magnitudes will only include a rough estimate or no
correction (along with an indication of what SED was assumed to
generate this value). However, for stars which will be used in
self-calibration (see subsection~\ref{sec:selfcalib}) to determine
photometric zeropoints in each exposure, a model SED well-matched to
the object's colors will be chosen and used to generate the $\delta
k_b^{atm+sys}$ corrections.
