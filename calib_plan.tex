...
The shape of the atmospheric transmission curve, $\phi^{atm}(\lambda, alt, az, t)$,
is determined by three major factors: molecular scattering (Rayleigh
scattering), aerosol scattering, and molecular absorption. Molecular
scattering, or Rayleigh scattering, is due to elastic scattering off
atoms and molecules in the air. Atmospheric absorption due to
rayleigh scattering is proportional to $ e^{-\tau X}$, where $\tau \propto BP\,
\lambda^{-4}$ and $BP$ is the barometric pressure. Aerosol scattering
is due to Mie scattering off particles suspended in the atmosphere,
where the particle sizes and wavelengths of light are comparable. This
gives rise to a variable absorption curve, with $\tau \propto
(\lambda/675 \rm{nm})^{\alpha}$ where $\alpha$ ranges from -0.9 to
-1.7 based on a limited set of observations from CTIO
\citep{Burke2010b}. Molecular absorption produces a more complex set
of transmission features, including 
