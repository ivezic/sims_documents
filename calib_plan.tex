...
the normalization of $S_b^{sys}$ that are than the PSF (and in the
current calibration implementation, smaller than the scale of a CCD).
...
source. This source's emittance would be a Lambertian with uniform surface
brightness independent of location on the sky, and it would
...
photometric flat at the aperture center.  The resulting magnitudes for
stars (of the selected SED) measured with this technique would be
uniform across the entire field of view. 

In practice, we do not have a point source of this nature to create
this ideal photometric flat. Typical astronomical flat fields generated from dome
screens, the twilight sky or dark sky flats all contain light sources
that illuminate the entire telescope field of view simulateously and
which are only approximately Lambertian.  In addition, these sources
are only approximately uniform over the field of view, and each has an
SED which differs from that of the source to be measured.
