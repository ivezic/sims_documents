...
subsection~\ref{sec:ic}) for each
...
'green' and 'red' wavelengths. The bottom panel shows the result of 
combining these flats with a desired SED that is either weighted
toward blue wavelengths, a completely even distribution, or weighted
...
There will be also light scattered within the camera dewar and some
fraction of the light within the etendue will have undergone multiple
reflections within the camera refractive optics (creating `ghost'
images of the light from the dome screen). This stray and scattered
light is not present in the science images and so must be removed from
the flat field using the illumination correction. In addition,
...
from the dome screen. This effect {\it is} present in the night sky
science images as well, but does not affect the flux measured from
astronomical objects and so this gradient must be preserved.
Illlustrative examples of these effects in a dome flat and the
...
XXXX START HERE - split IC generation up for clarity and remove(?) CCOB
